...
public class Connect4Frame extends JFrame
{
    ...
    private void nextMove()
    {
        ...
        if (won != 'N') // if the game has been won...
        {
            ...
            {
                alert(redPlayer.toString() + " wins! yay!! Thats ok yellow better luck next time");
            }
            else if (myGame.gameWon() == 'Y') // if yellow won, say so
            {
                alert(yellowPlayer.toString() + " wins! yay!! Thats ok red better luck next time");
            }
        }
        else if (myGame.boardFull()) // if the board is full...
        {
            disableButtons();   // disable the buttons
            alert("The game ended in a draw! You both win, or lose depending on how you look at life"); // announce the draw
            gameActive = false;
        }
        this.repaint();
    }
